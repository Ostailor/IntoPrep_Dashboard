...
Performance Task 5: complete with explicit evidence gaps (production-style local QA warm medians/p95 all under 72 ms FCP and 21 ms load; 0 idle sibling RSC, route-specific intent requests only; four-route/import-panel/export UI check clean; lint/typecheck/build pass, full suite 304/305 with only known QuickBooks failure; no valid numeric pre-optimization baseline and no literal Demo badge)
